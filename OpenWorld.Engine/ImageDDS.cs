...
namespace OpenWorld.Engine
{
	...
	partial class Texture
	{
		...
		private static NotImplementedException Unfinished { get { return new NotImplementedException("ERROR: Only 2 Dimensional DXT1/3/5 compressed images for now. 1D/3D Textures may not be compressed according to spec."); } }
		#endregion Constants
		...
		protected TextureTarget LoadDDS(Stream stream)
		{
			...
			try // Exceptions will be thrown if any Problem occurs while working on the file. 
			{
				...
				do
				{
					len = stream.Read(block, 0, block.Length);
					offset += len;

					data.AddRange(block);
				} while (len != 0);

				_RawDataFromFile = data.ToArray();
				Array.Resize(ref _RawDataFromFile, offset);
				...
				return dimension; // success
			}
			catch (Exception ex)
			{
				Log.Error("Exception caught when attempting to load dds texture.", ex);
				throw;
			}
		}
